...
public class RecurStar54 extends RecurFigure {
	private int size;
	private int depth;
	...
	public RecurStar54(int depth, int size) {
		this.depth = depth;
		this.size = size;
	}
	...
	private void recureFigure5(Graphics g, int x, int y,int size,int depth) {
		...
		recureFigure4(g,x-size+38,y-size,size/2,depth-1);
		recureFigure4(g,x+size/2+10,y+size-40,size/2,depth-1);
		recureFigure4(g,x-size+38,y+size,size/2,depth-1);
		recureFigure4(g,x-size-10,y+size-40,size/2,depth-1);
	}
	private void recureFigure4(Graphics g, int x, int y,int size,int depth) {
		if(depth <=0)
			return;
		drawStar5(g, x, y, size);
		recureFigure5(g,x+size/8,y-size,size/2,depth-1);
		recureFigure5(g,x-size/3,y+size,size/2,depth-1);
		recureFigure5(g,x-size,y-size/7,size/2,depth-1);
		recureFigure5(g, x+size,y-size/7,size/2,depth-1);
		recureFigure5(g,x+size-30,y+size,size/2,depth-1);
	}
